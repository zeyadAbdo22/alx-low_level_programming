...
/**
 * string_check - check if two strings are identical.
 ...
 */
int string_check(char *s1, char *s2, int i, int j)
{
	if (s1[i] == '\0' && s2[j] == '\0')
		return (1);
	if (s1[i] == s2[j])
		return (string_check(s1, s2, i + 1, j + 1));
	if (s1[i] == '\0' && s2[j] == '*')
		return (string_check(s1, s2, i, j + 1));
	if (s2[j] == '*')
		return (string_check(s1, s2, i + 1, j) || string_check(s1, s2, i, j + 1));
	return (0);
}
